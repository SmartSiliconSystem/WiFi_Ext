...
#if 1
        if (pRxBuff->pRcvMsgBuff[0] != MAGIC) pRxBuff->pWritePos = pRxBuff->pRcvMsgBuff;
        else if (pRxBuff->pRcvMsgBuff[1] != MAGIC) pRxBuff->pWritePos = pRxBuff->pRcvMsgBuff;
        else if ((pRxBuff->pWritePos - pRxBuff->pRcvMsgBuff) == MSG_SIZE-1)
        {
        	// end of message, copy pRxBuff->pRcvMsgBuff to MyAppliance with MSG_SIZE length
        	// Check crc
        	if (crc16_compute(pRxBuff->pRcvMsgBuff + 4, MSG_SIZE - 4, NULL) == (uint16_t)(pRxBuff->pRcvMsgBuff[2]))
        	{
        		os_memcpy(&MyAppliance, pRxBuff->pRcvMsgBuff, MSG_SIZE);
        		// update MyAppliance with data received from browser
        		MyAppliance.Status = WebData.Status;
